...
WHERE users.id IN (
    SELECT DISTINCT outgoing_msg_id FROM messages WHERE outgoing_msg_id IS NOT NULL
) OR users.id IN (
    SELECT DISTINCT incoming_msg_id FROM messages WHERE incoming_msg_id IS NOT NULL
